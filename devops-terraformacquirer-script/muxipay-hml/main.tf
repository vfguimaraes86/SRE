...
terraform {
  ...
  backend "s3" {
    bucket         = "640568149311-us-east-2-terraform-remote-backend-state"
    profile        = "muxipay-hml"
    key            = "muxipay-hml/terraform.tfstate" #### TODO #### nome da pasta no buket ({NAMESPACE}/terraform.tfstate)
    region         = "us-east-2"
    encrypt        = true
...
module "enviroment" {
  ...
  environment                     = "muxipay-hml"          #### TODO #### nome da conta
  namespace                       = "muxipay-hml"          #### TODO #### nome do namespace
  ...
  #eks_cluster_name                = "muxipay-hml"          #### TODO #### nome do cluster EKS (opicional)
  environmentConfig = {
