...
terraform {
  ...
  backend "s3" {
    ...
    profile        = "muxipay-dev"
    key            = "muxipay-gw-poc/terraform.tfstate" #### TODO #### nome da pasta no buket ({NAMESPACE}/terraform.tfstate)
    region         = "us-east-2"
    encrypt        = true
...
module "enviroment" {
  ...
  namespace                       = "muxipay-gw-poc"       #### TODO #### nome do namespace
  #force_destroy                   = true                   #### TODO #### se permite deletar secret e bucket instantaneamente (default false)
  domain                          = "poc"                     #### TODO #### dominio $http://{sub_domain}${"." + domain}.${route53.somain}
  eks_cluster_name                = "muxipay-poc"          #### TODO #### nome do cluster EKS (opicional)
  environmentConfig = {
